...
import { IConnection, Position, TextDocument, WorkspaceFolder } from "vscode-languageserver";
...
describe('updateFile', () => {
  ...
  const textDocument: TextDocument = {
    uri: `file://${Path.join(projectDirectory, 'main.ink')}`,
    languageId: "",
    version: 0,
    getText(range?: Range): string { return content },
    positionAt(offset: number): Position { return Position.create(0, 0) },
    offsetAt(position: Position): number { return 0 },
    lineCount: 0
  };
